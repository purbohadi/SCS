...
public class HeroTradeIn {
	...
	Scanner sc = new Scanner(new FileInputStream("inputHeroTradeIn.txt"));
	...
	for (int i = 0; i < phoneGroups[0].getComponentCount(); i++) {
		...
		phoneGroups[visitedGroup.get(j).getX()].getComponents()[visitedGroup.get(j).getY()].setVisited(false);
	    }
