...
public class InterstellarCargo {
    ...
    public static String[] preD, path;
	...
	for (int tes_case = 0; tes_case < T; tes_case++) {
	    ...
	    preD = new String[limit];

	    preD[0] = Integer.toString(x1) + ',' + y1;
	    preD[1] = Integer.toString(x2) + ',' + y2;
		...
		x2 = sc.nextInt();
		y2 = sc.nextInt();
		// weight = countWeight(x1, y1, x2, y2);
		nodes[idx][0] = x1;
		nodes[idx++][1] = y1;
		nodes[idx][0] = x2;
		nodes[idx++][1] = y2;
		graph.addEdge(idx - 2, idx - 1, -1);

	    }

	    for (int i = 0; i < limit; i++) {
		for (int j = 0; j < limit; j++) {
		    weight = countWeight(nodes[i][0], nodes[i][1], nodes[j][0],
			    nodes[j][1]);
